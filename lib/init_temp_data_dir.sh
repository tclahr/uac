...
_init_temp_data_dir()
{
  ...
  if [ -d "${__UAC_TEMP_DATA_DIR}" ] && printf "%s" "${__UAC_TEMP_DATA_DIR}" | grep -q "uac-data.tmp"; then
    rm -rf "${__UAC_TEMP_DATA_DIR}" >/dev/null 2>/dev/null \
      || { _error_msg "cannot remove old temporary data directory from previous collection '${__UAC_TEMP_DATA_DIR}'."; return 1; }
  fi

  # create temporary directory
  mkdir -p "${__UAC_TEMP_DATA_DIR}" >/dev/null 2>/dev/null \
    || { _error_msg "cannot create temporary data directory '${__UAC_TEMP_DATA_DIR}'."; return 1; }
